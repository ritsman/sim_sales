...
const PurchaseOrderList = () => {
  ...
  const fetchPurchaseOrders = async () => {
    try {
      ...
      const response = await axios.get(
        `${config.API_URL}/api/material/getPurchaseOrder`
      );
  ...
  const handleViewDetails = async (id) => {
    try {
      setLoading(true);
               let data = purchaseOrders.find(item=>item._id == id);
      setSelectedPO(data);
      setIsModalOpen(true);
      setLoading(false);
    } catch (err) {
  ...
  const handleEdit = (id) => {
    ...
    window.location.href = `/material/edit-purchase-order/${id}`;
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(true);
      await axios.delete(
        `${config.API_URL}/api/material/deletePurchaseOrder/${poToDelete._id}`
      );

      // Remove from state
      setPurchaseOrders(
        purchaseOrders.filter((po) => po._id !== poToDelete._id)
      );
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-6 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <NavLink to="/material/purchaseorder">
            ...
            <tbody>
              {purchaseOrders.map((po) => (
                <tr key={po._id} className="hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 border-b">{formatDate(po.date)}</td>
                  <td className="py-3 px-4 border-b">
                    {po.party?.companyName || "Unknown"}
                  </td>
                  <td className="py-3 px-4 border-b">{po.total.toFixed(2)}</td>
                  {/* <td className="py-3 px-4 border-b">
                    <span
                  ...
                  <td className="py-3 px-4 border-b">
                    <div className="flex space-x-2">
                      ...
                      <button
                        onClick={() =>
                          generatePDF(po, companyProfile, "PURHCASE ORDER","PO")
                        }
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50">
          <div className="relative bg-white rounded-lg shadow-lg p-6 max-w-4xl w-full">
              ...
              <table className="min-w-full bg-white">
                ...
                <tfoot className="bg-gray-50">
                  ...
                  <tr>
                    ...
                    <td className="py-2 px-4">{selectedPO.tax.toFixed(2)}</td>
                  </tr>
                  <tr>
            ...
            <div className="flex justify-end mt-4 space-x-2">
              <button
                onClick={() =>
                  generatePDF(selectedPO, companyProfile, "PURHCASE ORDER","PO")
                }
