...
const GRNView = () => {
  ...
  const fetchPurchaseOrders = async () => {
    try {
      ...
      const response = await axios.get(
        `${config.API_URL}/api/material/getGRN`
      );
  ...
  const handleViewDetails = async (id) => {
    try {
      setLoading(true);
      let data = GRN.find((item) => item._id == id);
      setSelectedPO(data);
      setIsModalOpen(true);
      setLoading(false);
    } catch (err) {
  ...
  const handleEdit = (id) => {
    ...
    window.location.href = `/material/edit-grn/${id}`;
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(true);
      await axios.delete(
        `${config.API_URL}/api/material/deleteGRN/${poToDelete._id}`
      );

      // Remove from state
      setGRN(
        GRN.filter((po) => po._id !== poToDelete._id)
      );
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-6 max-w-6xl mx-auto">
      ...
      {!loading && GRN.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-gray-500">No GRN found</p>
          <button
            onClick={() => (window.location.href = "/material/grn")}
            ...
            <tbody>
              {GRN.map((item) => (
                <tr key={item._id} className="hover:bg-gray-50">
                  <td className="py-3 px-4 border-b">{item.grnNumber}</td>
          ...
          <div className="relative bg-white rounded-lg shadow-lg p-6 max-w-4xl w-full">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <p className="font-medium">{selectedPO.grnNumber}</p>
              </div>
              <div>
