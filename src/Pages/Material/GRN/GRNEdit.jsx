   ...
   const fetchGRN = async () => {
     try {
       ...
       const response = await axios.get(
         `${config.API_URL}/api/material/getGRNById/${id.id}`
       );
       const grnData = response.data;
       console.log(response, "grn Data");

       setFormData({
         ...grnData,
          items:grnData.items || [],
         // Format dates for input fields
         date: new Date(grnData.date).toISOString().split("T")[0],
         deliveryDate: new Date(grnData.deliveryDate)
           .toISOString()
           .split("T")[0],
         partyLocation: grnData.partyLocation || "xyz",
       });
        ...
        <div>
          ...
          <div className="mb-4">
            ...
            <select
              ...
              onChange={handleInputChange}
              className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
              disabled={formData.category !== "purchase_order"}
            >
              <option value="">Select Purchase Order</option>
              {purchaseOrders.map((po) => (
                <option key={po._id} value={po._id}>
                  {po.grnNumber || po.poNumber}
                </option>
              ))}
            </select>
          </div>
