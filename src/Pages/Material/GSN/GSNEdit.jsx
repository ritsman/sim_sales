     ...
     const fetchGSN = async () => {
       try {
         ...
         const response = await axios.get(
           `${config.API_URL}/api/material/getGSNById/${id.id}`
         );
         const gsnData = response.data;
         console.log(response, "gsn Data");

         setFormData({
           ...gsnData,
           items: gsnData.items || [],
           // Format dates for input fields
           date: new Date(gsnData.date).toISOString().split("T")[0],
           deliveryDate: gsnData.deliveryDate
             ? new Date(gsnData.deliveryDate).toISOString().split("T")[0]
             : " ",
           partyLocation: gsnData.partyLocation || "xyz",
         });
       } catch (err) {
         console.error(err);
       }
    ...
    if (name === "party") {
      ...
      if (selectedPO) {
        const updatedItems = (
          ...
          const tax = item.tax || subtotal * TAX_RATE;

          return {
            ...item,
            id: item.id || Math.floor(Math.random() * 10000), // Ensure each item has an id
            itemId: item.itemId || item.productId || item._id, // Ensure proper ID reference
            tax: tax, // Ensure tax field is included
            amount: subtotal + tax, // Recalculate amount to include tax
