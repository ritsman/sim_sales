  ...
  const handleAfterChange = (changes, source) => {
    if (source === "edit") {
      ...
      changes.forEach(([row, property, oldValue, newValue]) => {
        if (property === "stock" && !isNaN(newValue) && newValue !== oldValue) {
  ...
  const columns = [
    {
      ...
      renderer: function (instance, td, row) {
        const itemId = items[row]?._id;
        td.innerHTML = `<input type='checkbox' ${
          selectedItems.includes(itemId) ? "checked" : ""
        }/>`;
    ...
    <div className="max-w-6xl mx-auto p-6 bg-white shadow-md rounded-lg">
      ...
      <div className="flex gap-5">
        ...
        <button
          ...
          className="mb-4 px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-800"
        >
          Delete Selected
        </button>
